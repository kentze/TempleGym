...
import type { SessionType, WorkoutSession, WorkoutsListResponse } from '@templegym/types';
...
export default function HomeScreen() {
  ...
  const {
    ...
    removeRoutineItem, duplicateRoutineItem, replaceRoutineItem,
  } = useRoutinesStore();
  ...
  function openCardMenu(folderId: number | 'default', index: number, pageY: number) {
  ...
  return (
    ...
    <ScrollView
      ...
      refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh} tintColor="transparent" colors={['transparent']} />}
    >
      {/* Header */}
      ...
      {/* Stats */}
      <View style={[styles.statsRow, (user?.weeklyPoints ?? 0) > 0 && { backgroundColor: tierOuterBg(user!.weeklyPoints) }]}>
        <View style={[styles.statCard, { borderRadius: 10, padding: 8 }]}>
          <Text style={styles.statValue}>{user?.totalPoints ?? 0}</Text>
          <Text style={styles.statLabel}>Total Points</Text>
        </View>
        <View style={styles.statDivider} />
        <View style={[styles.statCard, { backgroundColor: tierBg(user?.weeklyPoints ?? 0), borderRadius: 10, padding: 8, margin: 7 }]}>
              ...
              {routine.items.map((item, i) => (
                <View key={i} style={styles.routineCard}>
                  <View style={styles.routineCardHeader}>
                    ...
                    <TouchableOpacity
                      onPress={(e) => openCardMenu(routine.id, i, e.nativeEvent.pageY)}
                      hitSlop={8}
                    >
            ...
            {defaultItems.map((item, i) => (
              <View key={i} style={styles.routineCard}>
                <View style={styles.routineCardHeader}>
                  <Text style={styles.routineCardType}>{item.name}</Text>
                  <TouchableOpacity
                    onPress={(e) => openCardMenu('default', i, e.nativeEvent.pageY)}
                    hitSlop={8}
                  >
                    <Ionicons name="ellipsis-horizontal" size={18} color={Colors.textMuted} />
                  </TouchableOpacity>
      ...
      <Modal visible={planVisible} transparent animationType="fade">
        <Pressable style={styles.modalOverlay} onPress={() => { setPlanVisible(false); setRoutineExpanded(false); }}>
          <View style={styles.modalCard}>
            ...
            {/* Routine card */}
            <TouchableOpacity
              style={[styles.sessionCard, styles.routinePlanCard, routineExpanded && styles.planCardHighlight]}
              onPress={() => setRoutineExpanded((v) => !v)}
              activeOpacity={0.75}
            >
              <View style={[styles.sessionCardAccent, { backgroundColor: Colors.textMuted }]} />
              <View style={styles.sessionCardBody}>
            ...
            {routineExpanded && (
              allRoutineItems.length === 0
                ? <Text style={styles.planRoutineEmpty}>Create a routine from the home screen first.</Text>
            ...
            <TouchableOpacity style={styles.planCancelBtn} onPress={() => { setPlanVisible(false); setRoutineExpanded(false); }}>
    ...
    <Animated.View
      pointerEvents="none"
      style={[styles.refreshLogoWrap, { top: insets.top + 6, opacity: logoOpacity }]}
    >
      <Animated.Image
    ...
    {cardMenuKey !== null && (() => {
      ...
      const item = folderId === 'default'
        ? defaultItems[itemIndex]
        : folders.find((f) => f.id === folderId)?.items[itemIndex];
      const folderName = folderId === 'default'
        ? defaultRoutineName
        : (folders.find((f) => f.id === folderId)?.name ?? '');
      if (!item) return null;
      return (
        <Pressable style={styles.popupOverlay} onPress={() => setCardMenuKey(null)}>
          <View style={[styles.cardMenu, { top: cardMenuTop }]}>
            ...
            <TouchableOpacity
              style={styles.cardMenuItem}
              onPress={() => { duplicateRoutineItem(folderId, itemIndex); setCardMenuKey(null); }}
            >
            ...
            <TouchableOpacity
              style={styles.cardMenuItem}
              onPress={() => { removeRoutineItem(folderId, itemIndex); setCardMenuKey(null); }}
            >
              <Ionicons name="trash-outline" size={16} color={Colors.error} />
              <Text style={[styles.cardMenuText, { color: Colors.error }]}>Delete</Text>
            </TouchableOpacity>
          </View>
        </Pressable>
      );
...
const styles = StyleSheet.create({
  ...
  // Stats
  statsRow:              { flexDirection: 'row', backgroundColor: Colors.surface, borderRadius: 16, borderWidth: 1, borderColor: Colors.border, padding: 5 },
  ...
  statTier:              { fontSize: 11, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 2, marginBottom: 4 },
  ...
  routineCardType:       { fontSize: 17, fontWeight: '700', color: Colors.text, flex: 1 },
  ...
  refreshLogoWrap:       { position: 'absolute', left: 0, right: 0, alignItems: 'center', zIndex: 200 },
  refreshLogoImg:        { width: 38, height: 38, resizeMode: 'contain' },
  cardMenu:              { position: 'absolute', right: 20, backgroundColor: Colors.surface, borderRadius: 10, borderWidth: 1, borderColor: Colors.border, overflow: 'hidden', shadowColor: '#000', shadowOpacity: 0.18, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: 6, minWidth: 160 },
  cardMenuItem:          { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 16, paddingVertical: 12 },
  ...
  planRoutineItem:       { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.background, borderRadius: 10, borderWidth: 1, borderColor: Colors.border, paddingHorizontal: 14, paddingVertical: 10, gap: 10 },
  ...
  planRoutineEmpty:      { fontSize: 13, color: Colors.textMuted, fontStyle: 'italic', textAlign: 'center', paddingVertical: 8 },
});
